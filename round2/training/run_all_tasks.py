...
def get_task_lists():
    ...
    TRAIN_CONFIG_FILE = os.getenv('TRAIN_CONFIG_FILE', 
...
def evaluate_single_task(task_id, seed_list, snippet_metadata_path, best_runs_df):
    ...
    submission_data_path = os.getenv('SUBMISSION_DATA_PATH', 
                                     './example_data/submission_embedding.npy')

    eval = MABeSingleTaskEvaluator(task_id=task_id,
    ...
    eval.load_data()
    for seed in seed_list:
        best_run_path = best_runs_df[best_runs_df['task_id'] == task_id]['best_run_path']
